...
def cpplus_get_sd_card_info(camera_info):
    ...
    # ───────── SD CARD INFO ─────────
    urls = [
        f"https://{ip}/cpapi/storageDevice.cgi?action=getDeviceAllInfo",
        #f"http://{ip}/cgi-bin/storageDevice.cgi?action=getDeviceAllInfo"
    ]
...
# ─────────────────────────────────────────────
# MAIN LOOP (SEQUENTIAL SAFE VERSION)
# ─────────────────────────────────────────────
def cpplus_main_SDcard_Info():
    ...
    print("Camera IPs:", camera_info_list)
    ...
    if all_sd_card_entries:
        combined_data = {
            "Cpplus_SD_card_info": all_sd_card_entries
        }
        ...
        print("All SD card info sent successfully.")
        print(attributes_json)
...
def get_cpplus_cam_status(camera_info):
    ...
    try:
        ...
        # Even if response is weird, status 200 = device alive
        if response.status_code == 200:
            print(f"CPPlus camera is Active: {ip}")
            return "Active"

    except requests.exceptions.Timeout:
        #print(f"[TIMEOUT] CPPlus camera: {ip}")
        return "Inactive"

    except requests.exceptions.ConnectionError:
        #print(f"[CONNECTION ERROR] CPPlus camera: {ip}")
        return "Inactive"

    except requests.exceptions.HTTPError:
        #print(f"[HTTP ERROR] CPPlus camera: {ip}")
        return "Inactive"

    except Exception as e:
        #print(f"[ERROR] CPPlus camera {ip}: {e}")
        return "Inactive"
...
def main_cpplus_cam_status():
    ...
    print("Camera IPs extracted from CP Plus:", camera_info_list)
    ...
    for cam in camera_info_list:
        ...
        try:
            ...
            #print(f"Error processing {cam['ip_address']}: {e}")
            pass
    ...
    final_output = {
        "cpplus_camera_status": cpplus_camera_status
    }
...
if __name__ == "__main__":
    ...
    if logical_params_module.get_parameter("active_integration_cp_plus_nvr") != 1:
        ...
        _log.getLogger(__name__).info(
            "[CP_Plus_SD_Card_HDD_Recording.py] active_integration_cp_plus_nvr=0 "
    ...
    # ── INTERVAL tasks ────────────────────────────────────────────────────────
    schedule.every(3600).seconds.do(main_hik_cam_status)
    schedule.every(3600).seconds.do(main_dahua_cam_status)
    schedule.every(3600).seconds.do(main_cpplus_cam_status)

    # ── DAILY tasks: load-gated — replaces fixed clock times ─────────────────
    _daily_sent_date_cpplus_sd = None

    def maybe_send_daily_cpplus_sd():
        global _daily_sent_date_cpplus_sd
        today = datetime.today().date()
        if _daily_sent_date_cpplus_sd == today:
            return
        if not is_rpi_idle():
            return
        hik_main_SDcard_Record_Info()
        ...
        dahua_main_SDcard_Info()
        cpplus_main_SDcard_Info()
        _daily_sent_date_cpplus_sd = today

    schedule.every(60).seconds.do(maybe_send_daily_cpplus_sd)
