...
#
# hik_RTC_sync.py — Hikvision NVR RTC time-sync and battery-status monitor
...
def get_nvr_time() -> Optional[datetime]:
    """
    Query Hikvision NVR current time via ISAPI/System/time (HTTP GET).
...
def update_nvr_time(local_time: str) -> bool:
    """
    Push local_time string to the Hikvision NVR via ISAPI XML PUT.
...
def check_and_sync_time() -> None:
    """
    Compare RPi system time against Hikvision NVR time.
    - Always inserts hikvision_ntp=updated when NVR is reachable.
    ...
    if time_diff > DRIFT_TOLERANCE_SEC:
        ...
        if current_count >= MISMATCH_THRESHOLD:
            ...
            log.warning("[sync] %d consecutive mismatches — flagging batt_low",
                        MISMATCH_THRESHOLD)
            _insert({"hik_nvr_battery_status": "batt_low"})
...
if __name__ == "__main__":
    ...
    #   sudo systemctl restart dexter-rtc-hikvision
    if logical_params_module.get_parameter("active_integration_hikvision_nvr") != 1:
        import logging as _lg, sys as _sys
        _lg.getLogger(__name__).info(
            "[hik_RTC_sync.py] active_integration_hikvision_nvr=0 — integration disabled, exiting cleanly"
        )
        _sys.exit(0)
    check_and_sync_time()
