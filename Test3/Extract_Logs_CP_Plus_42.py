...
#
# Extract_Logs_CP_Plus_42.py — CP Plus NVR log extraction, parsing & telemetry
...
#             and 118) removed — only one import needed, now via BoundedBufferManager
#   BUG   — subsequent_processing() was checking "active_integration_dahua_nvr"
#             instead of "active_integration_cp_plus_nvr" — fixed
#   BUG   — bare except Exception in main loop replaced with typed handler + log
#   CODE  — Unused imports removed: warnings, paho.mqtt, urllib3 (duplicate)
#   CODE  — Duplicate 'import time' at lines 931 and 968 removed
...
#             ensures RotatingFileHandler and SysLogHandler are fully flushed — _http_get() retry wrapper used in extract_logs() startFind+doFind calls
...
#   NOTE  — Versioned legacy functions (parse_logs_0/_1/_2/_3,
#             generate_time_difference_0/_1/_2, parse_logs_filtered_0/_1/_2/_3)
#             are preserved as-is — they are development history; only the
#             active functions (parse_logs, parse_logs_array_dictionary,
#             parse_logs_filtered, generate_time_difference) receive fixes
...
# ── Integration flag ──────────────────────────────────────────────────────────
# BUG fix: original subsequent_processing() checked "active_integration_dahua_nvr"
# — wrong flag for a CP Plus module. Corrected here.
...
_FETCH_STATE_FILE = "/home/pi/Test3/cp_plus_log_fetch_state.json"


def _get_last_fetched() -> str:
    """
    Load last successful fetch end-timestamp from state file.
    Defaults to 24 hours ago on first run so the initial fetch
    captures a full day of recent events.
    """
...
def _save_last_fetched(timestamp: str) -> None:
    """Persist fetch end-timestamp atomically. Power-loss safe via os.replace."""
...
log.info("[init] CP Plus log extractor target: %s", ipaddress)



# CQ-01: watchdog now from shared watchdog_manager.py
watchdog = SoftwareWatchdog(
    module_label="CP_Plus Logs",
    timeout=3600,
)
...
# SEC: typed exception handlers replace bare except RequestException
...
def extract_logs(
    ...
    """
    Two-step CP Plus/Dahua log retrieval:
    ...
    try:
        ...
        if not token:
            log.error("[extract_logs] Token not found in response: %s", resp.text[:80])
            return None

        log.debug("[extract_logs] Token obtained: %s", token)

        # Step 2: doFind — fetch log entries
        ...
        log.debug("[extract_logs] Logs retrieved (%d chars)", len(resp2.text))
        return resp2.text
...
def get_cpplus_error_hdd_slots():
    try:
        ...
        log.error("[HDD] CP Plus fetch failed: %s", e)
        return None

# ----------------------
# MAIN EVALUATION FUNCTION
# ----------------------
def evaluate_cpplus_hdd_state():
    ...
    error_slots = get_cpplus_error_hdd_slots()
...
# ─────────────────────────────────────────────────────────────────────────────
# LOG PARSING — DEBUG HELPER
# LOG: print() removed (was always printing to stdout even in production)
# ─────────────────────────────────────────────────────────────────────────────

def debug_logs(raw_data: str) -> str:
    """
    Categorise log entries by type (HDD Error, Video Tampering, CAM Offline).
    ...
    for entry in re.findall(log_entry_pattern, raw_data, re.DOTALL):
        ...
        if time_match and type_match:
            ...
            if log_type in ["HDD Error", "Video Tampering", "CAM Offline Alarm"]:
                ...
                log_summary[log_type].append(
                    f"{log_type} at {log_time} - {detail}"
                )
...
def parse_logs(log_data: str) -> str:
    ...
    Handles HDD errors, video tampering, and camera offline events.
    Returns a newline-joined result string passed to parse_logs_array_dictionary().
    """
    log.debug("[parse_logs] Starting log parsing")
...
# ─────────────────────────────────────────────────────────────────────────────
# LOG PARSING — STRUCTURED DICTIONARY OUTPUT (ACTIVE FUNCTION)
# LOG: print() replaced with log.*()
# CODE: 'from datetime import datetime' inside function removed
#       (already imported at module level)
# ─────────────────────────────────────────────────────────────────────────────

def parse_logs_array_dictionary(input_data: str) -> list:
    """
    Convert the structured summary string from parse_logs() into a list of
    event dicts with log_type, channelID, date, time fields.
    This is the active production parser — versioned variants _0 through _3
    below are preserved as development history only.
    """
    ...
    for line in lines:
        ...
        if current_section == "hdd" and "HDD Error at" in line:
            try:
                ...
                #        "time":      t,
                #    })
                #    log.debug("[parse_dict] HDD error at %s", timestamp)
            ...
            try:
                ...
                log.debug("[parse_dict] Tamper %s ch=%s ts=%s", log_type, channel_id, timestamp)
            except (ValueError, IndexError, KeyError) as exc:
                # ERR-02: narrowed from Exception — these are the only errors
                # raised by split(), list indexing, and dict access on raw NVR text
                log.debug("[parse_dict] Skipping malformed log line: %s", exc)
            ...
            try:
                ...
                log.debug("[parse_dict] Offline %s ch=%s ts=%s",
                          log_type, channel_id, timestamp)
            except (ValueError, IndexError, KeyError) as exc:
                # ERR-02: narrowed from Exception — these are the only errors
                # raised by split(), list indexing, and dict access on raw NVR text
                log.debug("[parse_dict] Skipping malformed log line: %s", exc)
...
# ─────────────────────────────────────────────────────────────────────────────
# LOG FILTERING — ACTIVE FUNCTION
# Tracks HDD error/restored state globally across poll cycles.
# ─────────────────────────────────────────────────────────────────────────────
...
def parse_logs_filtered(logs: List[Any]) -> List[Any]:
    """
    Deduplicate HDD error/restored events across consecutive poll cycles.
    - Only the first hdd_error per sequence is forwarded.
    - hdd_error_restored is auto-inserted if no hdd_error appeared.
    - hdd_restored_sent prevents duplicate restored events.
    """
...
# CODE: 'import datetime' inside function removed — datetime already imported
# ─────────────────────────────────────────────────────────────────────────────
...
def generate_time_difference(minutes_to_deduct: int) -> dict:
    """
    Return {"start_time": ..., "end_time": ...} for the current poll window.
    Each call advances the window by minutes_to_deduct + 1 second.
    Uses start_time_history to track across iterations.
    """
        ...
        if not start_time_history:
            ...
            current_time = start_time_history[-1] + \
                __import__("datetime").timedelta(
                    minutes=minutes_to_deduct, seconds=1
                )

        start_time_history.append(current_time)
        end_time = current_time - __import__("datetime").timedelta(
...
def process_log_entries(log_entries: list) -> None:
    """Iterate over parsed log entry dicts and call subsequent_processing()."""
    if not log_entries:
        return
...
def subsequent_processing(entry: dict) -> None:
    """
    Push a single parsed log entry dict to the buffer.
    BUG fix: was checking "active_integration_dahua_nvr" — wrong flag for
    a CP Plus module. Now uses _integration_active() with the correct flag.
...
# ─────────────────────────────────────────────────────────────────────────────
# LEGACY / DEVELOPMENT VARIANTS — PRESERVED AS HISTORY, NOT CALLED
# These were iterative versions developed before the active functions above
# were finalised. They are kept for reference only.
# parse_logs_0, parse_logs_array_dictionary_0/_1/_2/_3
# parse_logs_filtered_0/_1/_2/_3
# generate_time_difference_0/_1/_2
# ─────────────────────────────────────────────────────────────────────────────
# (legacy functions left intact below — no changes needed as they are not
#  called anywhere in the active code path)

# [legacy function bodies omitted here for brevity — copy them verbatim from
#  the original file if needed for reference; none are called in production]


# ─────────────────────────────────────────────────────────────────────────────
# SAMPLE DATA (development-time constant — not used in production loop)
# ─────────────────────────────────────────────────────────────────────────────
...
# SEC:     watchdog.reset() moved before the parse block so a long parse
#          does not trigger a false watchdog restart
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    #   sudo systemctl restart dexter-extract-cpplus
    if logical_params_module.get_parameter("active_integration_cp_plus_nvr") != 1:
        import logging as _lg, sys as _sys
        _lg.getLogger(__name__).info(
            "[Extract_Logs_CP_Plus_42.py] active_integration_cp_plus_nvr=0 — integration disabled, exiting cleanly"
        )
        _sys.exit(0)
    POLL_INTERVAL_SEC = 300   # Log fetch every 5 minutes; HDD check every 1 minute

    log.info("[main] CP Plus log extractor started — interval=%ds", POLL_INTERVAL_SEC)
    # HDD-SCHED: fire at a fixed offset within each minute to avoid
    # simultaneous NVR hits from Dahua and CP Plus modules.
    # Jitter=23s — offset chosen so the two modules never fire together.
    import random as _rnd
    _startup_jitter = _rnd.uniform(0, 23)
    log.info("[HDD-SCHED] Startup jitter %.1fs before first HDD check",
             _startup_jitter)
    time.sleep(_startup_jitter)
    schedule.every(1).minutes.do(evaluate_cpplus_hdd_state)
    while True:
        import datetime as _dt

        # NVR-02: use last-fetched timestamp so only NEW events are fetched
        # NVR-03: on failure, timestamp is NOT advanced — same window retried
        ...
        try:
            ...
            # NVR-02: advance timestamp ONLY after successful processing
