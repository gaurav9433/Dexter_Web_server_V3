...
log = get_dual_logger('Dahua_SD_Card_HDD_Recording')
...
#def get_camera_info_list_old(device_type):
#    try:
#        with sqlite3.connect("/home/pi/Test3/device_config.db") as conn:
#            cursor = conn.cursor()
#            cursor.execute("SELECT * FROM device_parameters WHERE device_type = ?", (device_type,))
#            row = cursor.fetchone()
#            if row:
#                camera_ip_json = row[6]  # index 6 = 'camera_ip' column
#                return json.loads(camera_ip_json)
#            else:
#                print(f"[ERROR] No device found for type: {device_type}")
#                return []
#    except Exception as e:
#        print(f"[ERROR] Fetching camera info list failed: {e}")
#        return []
...
def dahua_main_SDcard_Record_Info():
    ...
    camera_info_list = get_camera_info_list("DahuaNVR1")
    if camera_info_list:
...
#                    print("Empty response received from camera: {}".format(camera_ip))
                ...
                try:
                    ...
                    if response_status_strg is not None and response_status_strg.text != "MORE":
                        log.error("All results fetched. Exiting loop.")
...
#            print("Failed to get device info from Camera Info", camera_ip)
        ...
        log.error("An error occurred while fetching SD Card info for camera {}: {}".format(ip, e))
        return None
...
def get_dahua_cam_status(camera_info):  
    ...
    try:       
        ...
        log.error("[cam_status] Connection error for %s: %s", ip, e)
        return "Inactive"

    except requests.exceptions.HTTPError as e:
        if e.response is not None and e.response.status_code == 404:
            # Camera is reachable but not a Dahua CGI device (e.g. Hikvision)
            log.warning("[cam_status] %s returned 404 — not a Dahua CGI device", ip)
            return "Unknown"
        log.error("[cam_status] HTTP error for %s: %s", ip, e)
        return "Inactive"

    except Exception as e:
        log.error("[cam_status] Error for %s: %s", ip, e)
        return "Inactive"
...
#    except ConnectionError as e:
        ...
        log.error("Error initializing Hikvision NVR: %s", e)
        return "Inactive"
...
# NVR info — base_url and session are placeholders here.
# They are rebuilt inside __main__ after credentials are loaded.
base_url = ""
...
if __name__ == "__main__":
    ...
    #   sudo systemctl restart dexter-sd-dahua
    if logical_params_module.get_parameter("active_integration_dahua_nvr") != 1:
        import logging as _lg, sys as _sys
        _lg.getLogger(__name__).info(
            "[Dahua_SD_Card_HDD_Recording.py] active_integration_dahua_nvr=0 — integration disabled, exiting cleanly"
        )
        _sys.exit(0)
    ...
    if not devices:
        log.error(
            "[Dahua_SD_Card_HDD_Recording.py] No DahuaNVR1 entry in device_config.db "
    ...
    # Rebuild base_url and session now that credentials are known
    base_url      = f'http://{ipaddress}/cgi-bin/mediaFileFind.cgi'
    ...
    # ── INTERVAL tasks ────────────────────────────────────────────────────────
    schedule.every(3600).seconds.do(main_hik_cam_status)    # Hikvision cam status — 1 hr
    schedule.every(3600).seconds.do(main_dahua_cam_status)  # Dahua cam status     — 1 hr
    schedule.every(3600).seconds.do(main_cpplus_cam_status) # CP Plus cam status   — 1 hr

    # ── DAILY tasks: load-gated (Strategy B) — replaces fixed clock times ─────
    _daily_sent_date_dahua_sd = None

    def maybe_send_daily_dahua_sd():
        global _daily_sent_date_dahua_sd
        today = datetime.today().date()
        if _daily_sent_date_dahua_sd == today:
            return  # already sent today
        if not is_rpi_idle():
            log.debug("[daily] RPi busy — deferring Dahua SD card daily tasks")
            return
        # RPi is idle — send all daily bulk data
        log.info("[daily] RPi idle — sending Dahua SD card daily data")
        hik_main_SDcard_Record_Info()    # Hikvision SD card recording info
        dahua_main_SDcard_Record_Info()  # Dahua SD card recording info
        hik_main_SDcard_Info()           # Hikvision SD card info
        dahua_main_SDcard_Info()         # Dahua SD card info
        cpplus_main_SDcard_Info()        # CP Plus SD card info
        _daily_sent_date_dahua_sd = today

    schedule.every(60).seconds.do(maybe_send_daily_dahua_sd)
