...
def fetch_device_info():
    ...
    row = c.fetchone()
    conn.close()
    return row if row else ('XX', 'BXXXX')
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'panel_number' in df.columns and 'batch_number' in df.columns:
                ...
                panel = str(int(panel_raw)).zfill(4) if pd.notnull(panel_raw) else 'XX'  
